...
export default function CVUploadPage() {
  ...
  // Watch the additionalInfo field for changes
  const additionalInfo = watch('additionalInfo', '');
  
  // Update word count when additionalInfo changes
  useEffect(() => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // and then send data to DeepSeek API for roadmap generation
      ...
      // Navigate to roadmap page after processing
      setTimeout(() => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (result.error?.includes('corrupted') || result.error?.includes('not recognized')) {
          throw new Error('The file appears to be corrupted or not properly formatted. Please try saving your CV as a different PDF or use a DOC/DOCX format.');
        }
      ...
      setUploadStatus({
        type: 'error',
        message: error instanceof Error ? error.message : 'Failed to parse CV. Please try again.'
      });
    } finally {
      setIsParsing(false);
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            {/* File Upload Section */}
            <div className="border-2 border-dashed rounded-xl p-8 text-center transition-colors" style={{borderColor: '#2434B3'}} onMouseEnter={(e) => (e.target as HTMLButtonElement).style.borderColor = '#FF4B36'} onMouseLeave={(e) => (e.target as HTMLButtonElement).style.borderColor = '#2434B3'}>
              <input
                type="file"
                id="cv-upload"
                accept=".pdf,.doc,.docx"
